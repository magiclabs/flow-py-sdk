...
class SignAndVerifyUserMessageExample(Example):
    def __init__(self) -> None:
        super().__init__(tag="V.1.", name="SignAndVerifyUserMessageExample", sort_order=601)
    ...
    async def run(self, ctx: Config):
        ...
        async with flow_client(host=ctx.access_node_host, port=ctx.access_node_port) as client:
            ...
            c_signature_1 = utils.CompositeSignature(account_address.hex(), 0, signature.hex())

            # signer 3
            signature = account_signers[2].sign_user_message(message)
            c_signature_2 = utils.CompositeSignature(account_address.hex(), 2, signature.hex())
